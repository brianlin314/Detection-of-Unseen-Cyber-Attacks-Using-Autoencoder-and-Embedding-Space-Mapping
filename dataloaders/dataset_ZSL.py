...
class VideoImageDataset(Dataset):
    def __init__(self, root=None, clip_len=16, embedding_map=None, seen_list=None, unseen_list=None, transform=None):
        self.root = root
        self.clip_len = clip_len
        self.resize_height = 64
        self.resize_width = 64
        ...
        for label in sorted(os.listdir(self.root)):
            class_folder = os.path.join(self.root, label)
            self.fnames.append(class_folder)
            # 判斷是不是 seen_list 中的類別
            label = remove_suffix(label)
            if label in seen_list:
    ...
    def __getitem__(self, index):
        first_frame = Image.open(self.fnames[index] + '/0.png')
        if self.transform:
        ...
        return buffer, embedding, idx, first_frame
