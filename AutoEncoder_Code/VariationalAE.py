...
def train(seen_df, unseen_df, num_epochs, batch_size, evaluate=False):
    ...
    for epoch in range(EPOCHS):
        ...
        save_model = 'cic_anomaly_vae.pth'
    ...
    if evaluate == True:
        ...
        with torch.no_grad():
            ...
            for data in test_loader_mixed:
                data = data.to(device)
                recon_batch, mu, logvar = model(data)
                error = torch.mean(torch.square(recon_batch - data), dim=1)
        ...
        # 找到最佳 threshold
        errors = [e.item() for e in reconstruction_errors_mixed]  
        unseen_ratio = round(len(unseen_df) / len(X_combined_test), 3)
        print(f"test_seen 資料數: {len(X_test)}, unseen 資料數: {len(unseen_df)}, unseen 資料異常比例: {unseen_ratio}")
        optimal_threshold = find_optimal_threshold(errors, tolerance=unseen_ratio)
        round_threshold = math.floor(optimal_threshold * 1000000) / 1000000 # 取小數點後5位
        print(f"Optimal Threshold: {round_threshold}")

        anomalies = [idx for idx, error in enumerate(reconstruction_errors) if error > round_threshold]
        anomalies_unseen = [idx for idx, error in enumerate(reconstruction_errors_unseen) if error > round_threshold]
        ...
        pure_seen_test_acc = (len(X_test) - len(anomalies)) / len(X_test)
        unseen_test_acc = len(anomalies_unseen) / len(unseen_df)
        # 確定各自測試集的大小
        size_seen = len(X_test)
        size_unseen = len(unseen_df)
        # 計算加權平均準確率
        combined_accuracy = (pure_seen_test_acc * size_seen + unseen_test_acc * size_unseen) / (size_seen + size_unseen)
        print(f'Combined Accuracy: {combined_accuracy}')
        print(f'pure_seen_test_acc: {pure_seen_test_acc}')
        print(f'unseen_test_acc: {unseen_test_acc}')

def find_optimal_threshold(errors, tolerance):
    threshold = np.percentile(errors, 100 * (1 - tolerance))
    return threshold
...
if __name__ == '__main__':
    ...
    all_attack_name = ['DDoS_LOIC-HTTP', 'DDoS_HOIC', 'DDoS_LOIC-UDP', 'DoS_SlowHTTPTest', 'DoS_Slowloris', 'DoS_Hulk', 'DoS_GoldenEye', 'BruteForce-XSS', 'BruteForce-Web', 'SQL-Injection', 'BruteForce-SSH', 'BruteForce-FTP', 'Infiltration', 'Botnet']
    unseen_attack_name = ['BruteForce-FTP', 'Infiltration']
    seen_attack_name = list(set(all_attack_name) - set(unseen_attack_name))
